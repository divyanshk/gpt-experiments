    ...
    def on_train_end(self, args, state, control, **kwargs):
        if self.profiler:
            ...
            if self.export_chrome:
                ...
                print("View at: chrome://tracing (load the trace.json file)")
...
class GRPOTrainingTracker(TrainerCallback):
    ...
    def plot_progress(self):
        ...
        if any(x is not None for x in self.metrics["train_loss"]):
            ...
            train_steps = [
                steps[i] for i, x in enumerate(self.metrics["train_loss"]) if x is not None
            ]
        ...
        if any(x is not None for x in self.metrics["eval_loss"]):
            ...
            eval_steps = [
                steps[i] for i, x in enumerate(self.metrics["eval_loss"]) if x is not None
            ]
        ...
        if any(x is not None for x in self.metrics["reward_scores"]):
            ...
            reward_steps = [
                steps[i] for i, x in enumerate(self.metrics["reward_scores"]) if x is not None
            ]
        ...
        if any(x is not None for x in self.metrics["kl_divergence"]):
            ...
            kl_steps = [
                steps[i] for i, x in enumerate(self.metrics["kl_divergence"]) if x is not None
            ]
        ...
        if any(x is not None for x in self.metrics["learning_rate"]):
            ...
            lr_steps = [
                steps[i] for i, x in enumerate(self.metrics["learning_rate"]) if x is not None
            ]
