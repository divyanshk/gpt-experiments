        ...
        if device == "cuda":
            ...
            if total_memory >= 40:  # High-end local setup
                config = {
                    ...
                    "device": "auto",
                    "max_memory": {str(i): f"{torch.cuda.get_device_properties(i).total_memory // (1024**3) - 2}GB" 
                ...
                config = {
                    "use_quantization": True,
                    "device": "auto",
                    "max_memory": {str(i): f"{max(4, torch.cuda.get_device_properties(i).total_memory // (1024**3) - 2)}GB" 
                                 for i in range(gpu_count)},
                    "generation_config": {
                        "max_new_tokens": 128,
                        "temperature": 0.7,
